...
namespace Bourse.State
{
    ...
    /// Date : 2023-03-15
    /// </summary>
    public class ProfilPetitBudget : ProfilABS
    {    /// <summary>
         /// Auteur : Claudel D. Roy
         /// Description : constructeur
         /// </summary>
        public ProfilPetitBudget(double dBalance, TransactionBoursiere compte)
        {
            _balance = dBalance;
            _compte = compte;
            Initialiser();
        ...
        {
            _commission = 30.00d;
            _limiteInfBalance = 0.00d;
            _limiteSupBalance = 500000.00d;
        }
        /// <summary>
        /// Auteur : Claudel D. Roy
        ...
        public override string Achat(double dMontant, double dMoyenneMob)
        {
            ...
            double dNbrAchat = 0.00d;
                ...
                if (dMontant < dMoyenneMob * 0.90d)
                {

                    dAchat = _balance * 0.90d;
                    dNbrAchat = dAchat / dMontant;

                    if (_balance - (dMontant * dNbrAchat) - _commission < _limiteInfBalance)
                    {
                        Console.WriteLine("Aucun achat possible");
                    }
                    else
                    {
        ...
        public override string Vente(double dMontant, double dMoyenneMob)
        {
                ...
                else
                {
                    ...
                    if (dMontant > dMoyenneMob * 0.90d)
                    {
                        ...
                        _NombreAchat = 0;
                        Console.WriteLine("Merci SmallBud");
                        _compte.Transaction = ChangementEtat();
                        return _balance.ToString();
                    }
                    else
